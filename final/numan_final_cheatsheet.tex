...
\AddToShipoutPicture*{%
\AtTextUpperLeft{%
\raisebox{6pt}{\makebox[\textwidth][r]{\scriptsize\textcolor{black!60}{Updated: 23 Sep 2026 08:00}}}%
}%
}
\begin{center}
...
\end{kbox}
Intuition: $H$ = curvature of the error bowl; positive-definite = exactly one bottom. \textbf{Cauchy--Schwarz} $\Rightarrow\det H\ge0$; $>0$ if $\ge2$ distinct $x_i$ $\Rightarrow$ unique global min.
...
Features = inputs $(x_1,x_2)$; plane $a_0+a_1x_1+a_2x_2=0\cap x_1x_2$-plane = \hl{decision boundary} (class 1 vs 2). Logistic $\sigma(z)=\tfrac1{1+e^{-z}}: \mathbb R\to[0,1]$; classifier $\sigma(a_0+a_1x_1+a_2x_2)$.
\end{sloppypar}
...
\textbf{Simpson} ($n=2m$ even, uniform): fits parabola on each pair. Why $O(h^4)$? Parabola matches value+slope+curvature (trap: only value+slope).
\begin{kbox}
\centering
...
\wk{Euler error idea:} Taylor gives local $O(h^2)$ per step; $N=(b-x_0)/h=O(1/h)$ steps accumulate to global $O(h)$.
...
\textbf{Projection (scalar length):} $\mathrm{proj}_{\bar b}\bar a=\tfrac{\bar a\cdot\bar b}{|\bar b|}=|\bar a|\cos\alpha$. Basis: $\mathrm{proj}_{\bar\imath}\bar a=a^x$, $\mathrm{proj}_{\bar\jmath}\bar a=a^y$. \ex{Worked:} $\bar a=(0,1)$, $\bar b=(2,0)$: $\mathrm{proj}_{\bar\jmath}\bar a=1$, $\mathrm{proj}_{\bar\imath}\bar b=2$.\\
\textbf{Area:} parallelogram $S=a^yb^x-a^xb^y=-\det$; area $=|S|=|\det|$. \ex{Worked:} $u=(3,1),v=(1,2)$: $S=1\cdot1-3\cdot2=-5$, area $5$. \textbf{Volume (3D):} $|\det|$ of the $3\times3$; \ex{Worked:} $\mathrm{diag}(1,2,3)$ volume $=6$.\\
...
\textbf{Laplace:} $\det A=\sum_j a_{kj}\mathrm{cofac}(a_{kj})$ (any row $k$). \textbf{Row swap} flips sign. \ex{Worked:} $A=[[1,2,3],[4,5,6],[7,8,10]]$: cofactors $2,2,-3$ $\Rightarrow$ $\det=1(2)+2(2)+3(-3)=-3$.\\
...
\textbf{LU:} $A=LU$ ($L$ unit-lower, $U$ upper; set $\ell_{ii}=1$). \ex{Worked (2$\times$2):} $[[1,2],[3,4]]=[[1,0],[3,1]][[1,2],[0,-2]]$ ($u_{22}=4-3\cdot2=-2$). \textbf{Det:} $\det A=\det L\cdot\det U$; here $1\cdot(-2)=-2$ \wk{Check} $=1\cdot4-2\cdot3$.\\
\ex{Worked (3$\times$3):} $[[1,2,3],[4,5,6],[7,8,10]]=[[1,0,0],[4,1,0],[7,2,1]][[1,2,3],[0,-3,-6],[0,0,1]]$ ($\ell_1{=}4,u_4{=}-3,u_5{=}-6,\ell_2{=}7,\ell_3{=}2,u_6{=}1$); $\det=1\cdot(-3)\cdot1=-3$.\\
...
Given $A=[[1,2,3],[4,5,6],[7,8,10]]$, expand along row 1.
Step 1 — cofactors: $\mathrm{cofac}(a_{11})=+(5\cdot10-6\cdot8)=50-48=2$; $\mathrm{cofac}(a_{12})=-(4\cdot10-7\cdot6)=-(40-42)=2$; $\mathrm{cofac}(a_{13})=+(4\cdot8-5\cdot7)=32-35=-3$.
Step 2 — assemble: $\det A=1(2)+2(2)+3(-3)=2+4-9=-3$ (matches LU $\det=1\cdot(-3)\cdot1$).
\end{sloppypar}
