...
\AddToShipoutPicture*{%
\AtTextUpperLeft{%
\raisebox{6pt}{\makebox[\textwidth][r]{\scriptsize\textcolor{black!60}{Updated: 23 Sep 2026 08:53}}}%
}%
}
\begin{center}
...
\begin{sloppypar}\raggedright
\textbf{Sample point:} $x_i^*=\alpha x_{i-1}+(1-\alpha)x_i$: $\alpha=1$ left, $0$ right, $\tfrac12$ mid. (Quiz7 $\alpha=0.2$; Lec16 $x_{i-1}+h_i/3\Rightarrow\alpha=\tfrac23$, $+2h_i/3\Rightarrow\alpha=\tfrac13$.)
...
\begin{sloppypar}\raggedright
\begin{itemize}
\item \textbf{Taylor:} $f(x)=\sum_{k=0}^n\tfrac{f^{(k)}(x_0)}{k!}(x-x_0)^k+R_n$, $|R_n|\le\tfrac{Md^{n+1}}{(n+1)!}$; odd fn: only odd powers (deg 8 $\to x^7$), bound $\tfrac{12^{2m+2}}{(2m+2)!}$.
...
\item Vandermonde $\det=\prod_{i<j}(x_j-x_i)\ne0$ (distinct nodes); DD: $f[x_i,x_{i+1}]=\tfrac{\Delta f}{\Delta x}$ (Newton form not required).
\end{itemize}
\end{sloppypar}
...
\ex{Separate ODEs (Lec20):} $y'=x/y\Rightarrow y\,dy=x\,dx\Rightarrow y^2=x^2+C_1$. Circle IVP $y'=-x/y$, $(2,2)$: $y^2/2+x^2/2=C=4$ (radius 2).\\
...
\ex{Row-swap + expand:} $D=\det[[5,6,7],[0,0,1],[8,9,10]]$: swap rows 1,2 (flip sign), expand along the zero-heavy row 1:\\
